...
\question\TFQuestion{F}{An OID is a signed 4-byte integer.}
...
\question\TFQuestion{T}{With default settings, a table that takes up 160KB on disk has exactly 20 pages.}
...
\question\TFQuestion{F}{The postgres documentation recommends disabling autovacuum to improve the performance of very large databases.}

\question\TFQuestion{F}{The autovacuum will perform a VACUUM FULL operation when needed to reduce the amount of disk space used by a table.}

\question\TFQuestion{F}{The \lstinline{pg_freespacemap} extension is required to run a VACUUM FULL instead of just a VACUUM.}

\question\TFQuestion{T}{Postgres can perform a VACUUM FULL on two different tables at the same time concurrently.}

\question\TFQuestion{T}{Postgres can perform a VACUUM on two different tables at the same time concurrently.}
...
\question\TFQuestion{F}{When you use the UPDATE command to modify a row, \lstinline{t_xmax} field will not be modified.}

\question\TFQuestion{F}{Running the DELETE command can modify the \lstinline{t_xmin} field of a row.}

\question\TFQuestion{T}{A single DELETE command can delete multiple rows from a table.}

\question\TFQuestion{F}{A row with \lstinline{t_xmin} set to 99 will be visible to a transaction with \lstinline{xid} of 90.}


\question\TFQuestion{F}{\lstinline{t_cid} and \lstinline{t_ctid} are synonyms.}
...
\question\TFQuestion{T}{Postgres uses a table's FSM to quickly determine which page it can insert a tuple into.}

\question\TFQuestion{T}{Postgres uses a table's VM to speed up VACUUMing.}

\question\TFQuestion{F}{If you are inserting large text strings into postgres, the postgres documentation recommends compressing them first in order to save space.}

\question\TFQuestion{F}{The DELETE command deletes tuples directly from the page files on the harddrive.}
...
\question\TFQuestion{F}{Increasing a table's \lstinline{fillfactor} will generally cause the UPDATE operation to go faster, but the SELECT operation to go slower.}

\question\TFQuestion{T}{Tables that have INSERTs but no UPDATEs should use a \lstinline{fillfactor} of 100, but for tables with many updates, it it is recommended to use a lower \lstinline{fillfactor}.}
...
\question\TFQuestion{F}{A page can have 32 bytes of header data if it contains a column with more than 8 non-NULL values.}
