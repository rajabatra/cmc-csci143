...
\question\TFQuestion{F}{A GROUP BY clause can always be implemented with either a GroupAggregate or a HashAggregate.}
...
\question\TFQuestion{F}{It's always possible to use a hash join.}
...
\question\TFQuestion{T}{It is possible to have a FOREIGN KEY constraint without an index on the source table/column(s).}
...
\question\TFQuestion{F}{Both the inner and outer sides of a nested loop join can be parallelized.}
\question\TFQuestion{F}{Both the inner and outer sides of a merge join can be parallelized.}
\question\TFQuestion{T}{Both the inner and outer sides of a hash join can be parallelized.}
